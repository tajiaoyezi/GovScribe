...
- **新增 Go 依赖**：`github.com/milvus-io/milvus/client/v2`（import `milvusclient`/`entity`/`index`，旧 `milvus-sdk-go` 已废弃）；`github.com/openai/openai-go/v3`（embedding 调用，导入路径必须带 `/v3`）；自写 rerank client（约 20 行）。要求 Go 1.24+。
